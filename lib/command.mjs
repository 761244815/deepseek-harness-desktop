...
export function resolveNpmCommand({
  ...
  const pathValue = env.Path ?? env.PATH ?? ''
  const candidates = [
    env.NODE_HOME && join(env.NODE_HOME, 'npm.cmd'),
    ...pathValue.split(delimiter).filter(Boolean).map(directory => join(directory, 'npm.cmd')),
    env.ProgramFiles && join(env.ProgramFiles, 'nodejs', 'npm.cmd'),
  ].filter(Boolean)
  const command = candidates.find(candidate => fileExists(candidate))
  if (command) return command

  throw new Error('npm is required. Install Node.js 22.19.0 or newer, then restart DeepSeek Harness Desktop.')
}
...
export function resolveHarnessLaunch(runtimePath, {
  ...
  const packageEntrypoint = join(runtimePath, 'node_modules', '@deepseek-ai', 'dsh', 'lib', 'bin.js')
  const builtEntrypoint = join(runtimePath, 'apps', 'cli', 'lib', 'bin.js')

  if (fileExists(packageEntrypoint)) {
    const command = resolveNodeCommand({ env, platform, fileExists })
    return { command, args: [packageEntrypoint, ...commonArgs], shell: false, mode: 'npm' }
  }
